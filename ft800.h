...
uint8_t cmd(uint32_t data);				/* command function (returns 0: when failed to execute command, ie. co-p. is busy) */
uint8_t cmd_execute(uint32_t data);		/* execute command (tries to execute command max. 255 times) */
